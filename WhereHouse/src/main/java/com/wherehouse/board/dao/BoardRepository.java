...
public class BoardRepository implements IBoardRepository  {
	...
	public HashMap<String, Object> SearchBoardList(int pnIndex) {
		...
		List<BoardVO> boardAllList = jdbcTemplate.query(query, new BoardVOMapper());		// 전체 게시글 목록.(BoardVO)
		List <BoardVO> boardList = new ArrayList<BoardVO>();		// 전체 게시글 내용 중에서 사용자에게 보여질 내용.
		
		ArrayList<String> userId = new ArrayList<String>();			// 보여질 게시글 목록 내 닉네임 검색 위함.
		
		HashMap<String, Object> resultBoard = new HashMap<String, Object>();		// list.jsp 내 보여질 전체 데이터.
		
		
		
		/* pnMaxSize : 게시글 전체 페이지 개수.(페이지네이션)
		 * pnEnd : 요청 받은 게시글 페이지 */
		int pnSize = ((int) Math.ceil(boardAllList.size()/10))+1;
		pnIndex = pnIndex * 10;
		int pnEnd = pnIndex + 9;
		
		/* 전체 게시글 목록에서 사용자 요청 페이지에 해당하는 게시글 목록 가져오기. */
	...
	/* 게시글 목록에서 게시글 선택 시 해당 게시글 내용 상세를 BoardVO 객체로 바노한. */
	@Override
